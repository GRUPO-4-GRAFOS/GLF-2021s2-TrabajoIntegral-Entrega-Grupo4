export function operator(distc, locale, info_agr2){
    ...
    let agr_data, agr_data2 = [], pdata = [];
    let coord = [], coordf = [], coordx = [], coordy = [], local_coord = [], Aux_Coord = 0;
    let lngth = [], lngth_dne = [];
    let n_operativos = 1;
    let cont = 0;
    let x, y, x_1, y_1;
    ...
    for(let i = 0; i < (Object.keys(agr_data2).length); i++)
    {
        ...
        for(let j = 0; j < (Object.keys(agr_data2[i]).length); j++)
        {
            ...
            else if(j == 2)
            {
                ...
                info_msgs.innerHTML += Warn;
                console.log(`La distancia recorrida por el camión nº${n_operativos} es de ${lngth_dne[i]}`);

                Aux = agr_data2[i][j];

                if(agr_data2[i+1][j] == 0 || agr_data2[i][j+1] == 0 || agr_data2[i+1][j+1] == 0){
                    break;
                }
                else if(Aux <= 1000 && (agr_data2[i+1][j]+Aux) <= 1000)
                {
                    ...
                    info_msgs.innerHTML += Warn;
                    console.log(`El siguiente camión con espacio disponible para envíos es el nº${n_operativos}`);
                ...
                else
                {
                    ...
                    console.log(`No hay espacio en este camión para el proximo envío, <br> El envío será tomado por el camión nº${n_operativos}`);
